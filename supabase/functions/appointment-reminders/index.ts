  ...
  try {
    ...
    for (const settings of settingsList as BusinessSettings[]) {
      ...
      if (!company.evolution_instance_name) {
        console.log(`[appointment-reminders] No WhatsApp instance for company ${company.id}`);
        continue;
      }

      // Calculate the time window for reminders
      // We look for appointments that start within the reminder window
      const reminderMinutes = settings.appointment_reminder_minutes || 30;
      
      // Window: from now to now + reminderMinutes + 5 (buffer)
      // This ensures we catch appointments even with cron timing variations
      const windowStart = new Date(now.getTime());
      const windowEnd = new Date(now.getTime() + (reminderMinutes + 5) * 60 * 1000);
      
      // But we only want to send reminders for appointments that are
      // exactly within the reminder window (±2 min tolerance)
      const reminderTargetTime = new Date(now.getTime() + reminderMinutes * 60 * 1000);
      const reminderWindowStart = new Date(reminderTargetTime.getTime() - 3 * 60 * 1000); // 3 min before
      const reminderWindowEnd = new Date(reminderTargetTime.getTime() + 3 * 60 * 1000); // 3 min after
      ...
      if (targetsToSend.length === 0) {
        console.log(`[appointment-reminders] No new reminders to send for company ${company.id}`);
        continue;
      }

      console.log(`[appointment-reminders] Sending ${targetsToSend.length} reminders for company ${company.id}`);
      ...
      try {
        ...
        if (!n8nResponse.ok) {
          ...
          console.log(`[appointment-reminders] Successfully sent to n8n for company ${company.id}`);
